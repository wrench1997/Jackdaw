...
exports.postEx = async function (requestmeta) {
  ...
  try {
    ...
    //await this.client.Network.responseReceived(responseListener);

    await this.client.Page.navigate({ url: requestmeta.url });
    // console.log(`HTTP url:${requestmeta.url} \n status:${status}`);
...
exports.getEx = async function (requestmeta) {
  ...
  try {
    ...
    const bodys = await this.getOutHtml(5000)
    const isError = status < 200 || status >= 300;
...
exports.getOutHtml = async function (timeout) {
  const callback = async ({ requestId }) => {
    // const { response } = await this.client.Network.getResponseBodyForInterception({ requestId });
  ...
  //Finally, get the html page rendered by the js script once
  const selector = 'html';
  ...
  for (const nodeId of selection.nodeIds) {
    //const { attributes } = await this.client.DOM.getAttributes({ nodeId });
    //console.log(`NodeID: ${nodeId}:`, attributes);
