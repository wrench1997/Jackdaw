 ...
 */
exports.waitForPageToLoad = async function (timeout,callback=null) {
...
exports.postEx = async function (requestmeta, timeout = 5000) {
  browserIsInitialized.call(this);
      //清空浏览器缓存，比免浏览器加载已知缓存而不请求网络链接而失效
      await this.client.Network.clearBrowserCache()
      const fetchListener = async ({ requestId, request }) => {
        // console.log('HTTP状态码:', responseStatusCode);
        if (requestmeta.url == request.url) {
          await this.client.Fetch.continueRequest({
            requestId: requestId,
            url: requestmeta.url,
            method: requestmeta.method,
            postData: requestmeta.postData,
      ...
      this.client.on('Fetch.requestPaused', fetchListener);
      var status = null;
      var statusText = null;
      var headers  = null;
      var body = null;
      ...
      const responseListener = async ({ response }) => {
        if (response.url == requestmeta.url) {
      ...
      await this.client.on('Network.responseReceived', responseListener);

  try {
    await this.client.Page.navigate({ url: requestmeta.url });
    const body1 = await this.getOutHtml(timeout)
    const isError = status < 200 || status >= 300;
    body = body1;
    err = isError ? `Request failed with status ${status}` : null;
  } catch (err) {
    console.error(err);
  }
  ...
  try{
    ...
    var statusText = null;
    var headers = null;
    const body2 = await this.getDomHtml(timeout);
    const isError = status < 200 || status >= 300;
    err = isError ? `Request failed with status ${status}` : null;
    if (body2 !== "") { 
      body = body2;
    } else {
      // Handle the case where body2 is empty (optional)
      console.warn("body2 is empty!"); 
      // You might want to add error handling or other logic here
    }
   
  }catch (err) {
    console.log('getDomHtml error:', err);
  }

  // 清除事件监听器
  this.client.removeAllListeners('Fetch.requestPaused');
  this.client.removeAllListeners('Network.responseReceived');

  return { body: body, status: status, statusText: statusText, headers: headers, err: err }
}


exports.getEx = async function (requestmeta, timeout = 5000) {
  browserIsInitialized.call(this);
    //清空浏览器缓存，比免浏览器加载已知缓存而不请求网络链接而失效
    await this.client.Network.clearBrowserCache()
    const fetchListener = async ({ requestId, request }) => {
  ...
  try {
    await this.client.Page.navigate({ url: requestmeta.url });
    const body = await this.getOutHtml(timeout);
    bodys.push(body);
    const isError = status < 200 || status >= 300;
    const err = isError ? `Request failed with status ${status}` : null;
  } catch (err) {
    console.error(err);
  }
  //获取Dom的数据
  ...
  try{
    ...
    var statusText = null;
    var headers = null;
    const body = await this.getDomHtml(timeout);
    bodys.push(body);
    const isError = status < 200 || status >= 300;
    const err = isError ? `Request failed with status ${status}` : null;
    err__ = err
    ...
    this.client.removeAllListeners('Fetch.requestPaused');
    this.client.removeAllListeners('Network.responseReceived');
    return { body: bodys, status: status, statusText: statusText, headers: headers, err: err__ }
}


/**
 * Waits for a networkEventLoadingFinished. Throws error after timeout                        
 * @param {number} timeout - The timeout in ms. (Default: "loadPageTimeout" property in the browser instance options)
 */
// Network.${ eventName }
exports.waitForEventLoadingFinished = async function (timeout, callback) {
  return promiseTimeout(new Promise((resolve, reject) => {
    const listener = (...params) => {
      debug(`:: Network.loadingFinished => Network.loadingFinished!`)
      //this.removeListener('Network.loadingFinished', listener)
      resolve(callback(...params))
    }
    this.on('Network.loadingFinished', listener)
  }), timeout || this.options.browser.loadPageTimeout)
}


exports.waitRequestIntercepted = async function (timeout, callback) {
  return promiseTimeout(new Promise((resolve, reject) => {
    const listener = (...params) => {
      debug(`:: Network.requestIntercepted => Network.requestIntercepted!`)
      //this.removeListener('Network.loadingFinished', listener)
      resolve(callback(...params))
    }
    this.on('Network.requestIntercepted', listener)
  }), timeout || this.options.browser.loadPageTimeout)
}


/**
 * Get The current page responsebody. Throws error after timeout
 * @param {number} timeout - The timeout in ms. (Default: "loadPageTimeout" property in the browser instance options)
 */
exports.getOutHtml = async function (timeout) {
  const callback = async ({ requestId }) => {
  ...
  var responses = [];
  for (var i = 0; i < 1; i++) {
    //const { body } = await this.waitForPageToLoad(options.timeout,callback);
    const { body } = await this.waitForEventLoadingFinished(timeout, callback)
    responses.push(body)
  }
...
exports.getDomHtml = async function(timeout) {
  ...
  const selection = await this.client.DOM.querySelectorAll({ nodeId: root.nodeId, selector });
  for (const nodeId of selection.nodeIds) {
    const { outerHTML } = await this.client.DOM.getOuterHTML({ nodeId });
    responses.push(outerHTML);
  }
