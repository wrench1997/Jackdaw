...
exports.postEx = async function (requestmeta) {
  browserIsInitialized.call(this);
  ...
  try {
    ...
    const body1 = await this.getOutHtml(1500)
  ...
  try{
    ...
    var statusText = null;
    var headers = null;
    const body2 = await this.getDomHtml(1000);
    const isError = status < 200 || status >= 300;
    err = isError ? `Request failed with status ${status}` : null;
    if (body2 !== "") { 
      body = body2;
...
exports.getEx = async function (requestmeta) {
  browserIsInitialized.call(this);
    //清空浏览器缓存，比免浏览器加载已知缓存而不请求网络链接而失效
    await this.client.Network.clearBrowserCache()
    const fetchListener = async ({ requestId, request }) => {
  ...
  try {
    await this.client.Page.navigate({ url: requestmeta.url });
    const body = await this.getOutHtml(1500);
    bodys.push(body);
    const isError = status < 200 || status >= 300;
  ...
  try{
    ...
    var statusText = null;
    var headers = null;
    const body = await this.getDomHtml(1000);
    bodys.push(body);
    const isError = status < 200 || status >= 300;
    const err = isError ? `Request failed with status ${status}` : null;
    err__ = err
