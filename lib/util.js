...
exports.promiseTimeout = function (promise, timeout) {
  ...
  return new Promise((resolve, reject) => {
    timer = setTimeout(() => {
      ...
      // Throw the error but handle it within the same promise chain
      reject(err);

      // Optional: Perform additional actions after timeout (e.g., logging)
      console.warn('Promise timed out after', timeout, 'ms');
    }, timeout);

    promise.then(resolve, (error) => {
      clearTimeout(timer);
      reject(error); // Reject with the original error from the promise
    });
  });
};
